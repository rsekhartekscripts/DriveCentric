	...
	describe('Enterprise User - User Management Permissions', () => {
		...
		it('Test 11 - Validate Notification section permissions check boxes are disabled', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				//Check the Grab It! button is checked, if yes uncheck and click on update.
				...
				//Check the Grab It! button is unchecked
				cy.get(UserManagementElements.dialog_main_div).contains("Grab It!").within(() => {
					cy.get("md-switch").should('have.attr', 'aria-checked').then((attr) => {
		...
		it('Test 12 - Validate Notification section Check box enabled Permissions', function () {
			//Click on EO Menu
			cy.get(LoginLogoutElements.profile_menu).should('be.visible').click()
		...
		it('Test 13 - Validate Visit section perfmissions check boxes are disabled', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				//get initial values of all checkboxes
				//Check the Delete Visit button is checked, if yes uncheck and click on update.
				...
				//Check the Delete Visit button is unchecked
				cy.get(UserManagementElements.dialog_main_div).contains("Delete Visit").within(() => {
					cy.get("md-switch").should('have.attr', 'aria-checked').then((attr) => {
						expect(attr).to.equal("false")
					})
		...
		it('Test 14 - Validate Visit section Check box enabled Permissions', function () {
			//Click on EO Menu
			cy.get(LoginLogoutElements.profile_menu).should('be.visible').click()
