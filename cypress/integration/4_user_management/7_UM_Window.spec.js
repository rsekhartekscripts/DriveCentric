...
context('User Management', () => {
	...
	describe('Enterprise User - User Management Permissions', () => {
		...
		it('Test 33 - Update User Phone numbers', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				//Update Work phone field
				cy.wait(4000)
				cy.get(UserManagementElements.user_details_phone).clear({
					force: true
				}).click().type(UserManagementElements.user_details_phonenumber).should('have.value', UserManagementElements.user_details_phonenumber)
				//check if Cell phone field
				//cy.wait(4000)
		...
		it('Test 34 - Update Cell numbers', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				//Update Cell phone field
				cy.wait(4000)
				cy.get(UserManagementElements.user_details_cell).clear({
					force: true
				}).click().type(UserManagementElements.user_details_cellnumber).should('have.value', UserManagementElements.user_details_cellnumber)
				cy.wait(4000)
				cy.get(".driveDialogActions li:nth-of-type(2) button").should('be.visible').click()

			})
		})
		it('Test 35 - Update Drive Alias email', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				//Update Cell phone field
				cy.wait(4000)
				cy.get(UserManagementElements.user_details_alias).clear({
					force: true
				}).click().type(UserManagementElements.user_details_drivealias).should('have.value', UserManagementElements.user_details_drivealias)
				cy.wait(4000)
				cy.get(".driveDialogActions li:nth-of-type(2) button").should('be.visible').click()

			})
		})
		it('Test 36 - Validate CANCEL Button', function () {
			...
			cy.wait("@legacy").then((xhr) => {
				...
				cy.wait(4000)
				cy.get(".driveDialogActions li:nth-of-type(1) button").should('be.visible').click()
				cy.get(".driveDialogMessage .driveDialogAlert").contain('have.text', 'You have unsaved changes, are you sure you wish to cancel those changes and close the dialog?')
